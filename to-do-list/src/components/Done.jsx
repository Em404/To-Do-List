...
export const Done = () => {
  ...
  const handleDeleteSelectedToDo = () => {
    const newToDos = doneToDos.filter(
      (_, index) => !selectedToDo.includes(index)
    );
    const deletedToDo = doneToDos.filter((_, index) =>
  ...
  const handleRestoreSelectedToDo = () => {
    const newDoneToDos = doneToDos.filter(
      (_, index) => !selectedToDo.includes(index)
    );
    const restoredToDos = doneToDos.filter((_, index) =>
      selectedToDo.includes(index)
    );
  ...
  return (
    <Fragment>
      <div className="flex flex-col justify-between h-full">
          {doneToDos.length === 0 ? (
            <p className="text-center dark:text-white text-purple-800 font-semibold text-xl mt-4">No done tasks</p>
          ) : (
            <Fragment>
              {/* select */}
